<template>
  <div>
    ...
    <div style="margin-top: 10px;">
      <el-input @keyup.enter.native='doSearch' v-model="searchInput" placeholder="请输入书籍/出售人名称" class="searchInput"
        clearable>
    ...
    <div style="margin-top: 10px;">
      <el-table ref="singleTable" :data="tableData.slice((current_page-1)*per_page,current_page*per_page)"
        ...
        <el-table-column property="book_publishtime" label="出版时间" width="145">
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="180">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="danger" @click="deleteShop(scope.row.book_id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <div style="margin-top: 10px;">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="current_page"
        :page-sizes="[1 , 5, 10, 20]" :page-size="per_page" layout="total, sizes, prev, pager, next, jumper"
        :total="total">
      </el-pagination>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      async loadData() {
        ...
        const res = await this.$axios.post(`/myshoplist?user_id=${userID}`, {
          params
        })
        // 数据请求已经结束
        this.loading = false
      ...
      async doSearch() {
        ...
        const userID = sessionStorage.getItem('data')
        const res = await this.$axios.get(`/myshopsearch?user_id=${userID}`, {
          params
        })
        if (res.data.status === 200) {
          // 表格数据
