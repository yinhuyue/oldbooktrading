  ...
  <div>
    ...
    <!-- <div class="comment" style="margin-top: 20px">
      <div class="comment-list"
        v-for="(item,index) in bookDetailCommentMessage.slice((current_page-1)*per_page,current_page*per_page)"
        :key="item.comment_id">
        ...
        <div class="comment-list-content">
          评论内容：{{item.comment_content}}
        </div>
        <div class="comment-list-time">
          评论时间：{{item.comment_createtime | fmtDate('YYYY-MM-DD HH:mm:ss')}}
        </div>
      </div>
    </div> -->
    ...
    methods: {
      ...
      async loadData() {
        ...
        if (res3.data.status === 200) {
          // this.addShopForm.book_state = res3.data.data.book_state
          // this.addShopForm = res3.data.data
      ...
      async addComment() {
        ...
        this.$refs.addCommentRef.validate(async (valid) => {
          ...
          if (!valid) {
            ...
            const userID = sessionStorage.getItem('data')
            // const bookID = this.addCommentForm.book_id
            // const comment_content = this.addCommentForm.comment_content
            // const res = await this.$axios.post(`/useraddcomment?user_id=${userID}&book_id=${bookID}&comment_content=${comment_content}`)
