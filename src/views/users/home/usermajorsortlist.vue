<template>
  <div>
    <el-breadcrumb separator="/" style="padding-top: 8px; margin-bottom: 15px;">
      ...
      <el-breadcrumb-item>{{sortName.majorsort_name}}书籍列表</el-breadcrumb-item>
    </el-breadcrumb>
...
<script>
  export default {
    ...
    methods: {
      getId() {
        this.majorsort_id = this.$route.params.majorsort_id
      },
      async loadData() {
        // 数据请求开始
        this.loading = true
        ...
        const res = await this.$axios.post(`/usermajorsortbooklist?id=${this.majorsort_id}`, {
          params
        })
        this.loading = false
